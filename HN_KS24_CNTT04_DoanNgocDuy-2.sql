...
select r.room_type, count(b.room_id) as booking_times from rooms r join bookings b on r.room_id = b.room_id group by b.room_id;

-- phan 2:

-- Hãy liệt kê danh sách các lần đặt phòng
select g.guest_name, r.room_type, b.check_in from bookings b join guests g on b.guest_id = g.guest_id join rooms r on b.room_id = r.room_id;

-- Cho biết mỗi khách đã đặt phòng bao nhiêu lần
select g.guest_id, g.guest_name, count(b.guest_id) as booking_times from guests g join bookings b on g.guest_id = b.guest_id group by b.guest_id;
...
select g.guest_id, g.guest_name, count(b.guest_id) from guests g join bookings b on b.guest_id = g.guest_id group by g.guest_id, g.guest_name having count(b.guest_id) >= 2;