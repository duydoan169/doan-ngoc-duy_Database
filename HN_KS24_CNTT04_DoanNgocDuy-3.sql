...
select d.DeptName, count(s.StudentID) as number_of_students
...
group by DeptName
having max(number_of_students) = (select max(number_of_students) 
...
begin
    ...
    and score = (select max(score) from Enrollment
    where CourseID = p_CourseID
    group by CourseID);
end $$;
